...
export default function TeamModern() {
  // We only want the 7 leadership members. In the new data, they are the first 7.
  // We'll extract and order them: Commissioner, CEO, COO, CMO, CFO, CSO, CTO.
  const leadership = [
    teamMembers.find(m => m.role === "Commissioner"),
    teamMembers.find(m => m.role === "CEO"),
    teamMembers.find(m => m.role === "COO"),
    teamMembers.find(m => m.role === "CMO"),
    teamMembers.find(m => m.role === "CFO"),
    teamMembers.find(m => m.role === "CSO"),
    teamMembers.find(m => m.role === "CTO"),
    ...
    <div className="h-full bg-[#1A1A1A] rounded-3xl p-8 lg:p-12 border border-[#333] relative overflow-hidden group">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-4 relative z-10 items-stretch">
        {leadership.map((member, idx) => (
          <Link href={`/team/${member.id}`} key={idx} className="block group/member cursor-pointer h-full">
            <motion.div 
              whileHover={{ y: -5 }}
            ...
            >
              ...
              <div className="flex flex-col flex-grow items-center w-full">
                ...
                <p className="text-[#c084fc] text-[10px] md:text-xs font-semibold uppercase tracking-wider mt-auto pt-2">{member.role === "Commissioner" ? "Komisaris" : member.role}</p>
              </div>
            </motion.div>
          </Link>
        ))}
